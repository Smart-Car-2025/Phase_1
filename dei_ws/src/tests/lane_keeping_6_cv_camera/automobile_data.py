...
MIN_SPEED = -10                   # [m/s]     minimum speed
MAX_SPEED = 10                     # [m/s]     maximum speed
MAX_STEER = 24                   # [deg]     maximum steering angle
MAX_DSTEER = np.deg2rad(60.0)       # [rad/s]   maximum steering speed

# Vehicle parameters
LENGTH = 0.3  			            # [m]       car body length
...
class Automobile_Data():
    def __init__(self,
        ...
        self.obstacle_ahead_buffer = collections.deque(maxlen=10)   # FIFO queue for median filter on the encoder speed measurement
        self.obstacle_ahead_median = 3.0
        self.obstacle_ahead1 = 0.0   # [m]       SONAR:distance of an obstacle ahead
        self.obstacle_ahead_buffer1 = collections.deque(maxlen=10)   # FIFO queue for median filter on the encoder speed measurement
        self.obstacle_ahead_median1 = 3.0
        ...
        if trig_sonar1:
            self.sub_son1 = rospy.Subscriber("/automobile/sonar1", Float32, self.sonar_callback1)
    ...
    def sonar_callback1(self, data):
        self.obstacle_ahead1 = data.data if data.data>0 else self.obstacle_ahead1
        self.obstacle_ahead_buffer1.append(self.obstacle_ahead1)
        self.obstacle_ahead_median1 = np.median(np.array(list(self.obstacle_ahead_buffer1)))
