...
overcar = 0
overcar_state = None
overcar_start_time = None
is_overcar_handle = False

# Hàm x? lý giao l? không ch?n
intersection_actions = {
    1: [
        {"angle": 0, "speed": -6, "time": 0.85},
        {"angle": 25, "speed": -6, "time": 2.7},
    ],
    2: [
        {"angle": 0, "speed": -6, "time": 0.45},
        {"angle": 25, "speed": -6, "time": 0.25},
    ],
    3: [
        {"angle": 0, "speed": -6, "time": 1.7},
        {"angle": -25, "speed": -6, "time": 1.8},
    ],
    4: [
        {"angle": 5, "speed": -6, "time": 1.45},


    ],
    5: [
        {"angle": 0, "speed": 0, "time": 3},
        {"angle": 0, "speed": -6, "time": 1.45},
        {"angle": -25, "speed": -6, "time": 1.4},
    ],
    6: [
        {"angle": 0, "speed": -6, "time": 2.1},
        {"angle": 25, "speed": -6, "time": 3.3},
    ],
    7: [
        {"angle": 0, "speed": -6, "time": 1.15},
        {"angle": 25, "speed": -6, "time": 2.8},
    ],
    8: [
        {"angle": 0, "speed": 0, "time": 3},
        {"angle": 25, "speed": -6, "time": 0.5},
        {"angle": 0, "speed": -6, "time": 1.6},
    ],
    9: [
        {"angle": 0, "speed": -6, "time": 0.8},
        {"angle": 25, "speed": -6, "time": 2.88},
    ],
    10: [
        {"angle": 0, "speed": -6, "time": 1.3},
    ],
    11: [
        {"angle": 0, "speed": 0, "time": 10},
    ],
}
# Khai báo global
overcar_action = {
    1: [
        {"angle": -25, "speed": -6, "time": 0.9},
        {"angle": 25, "speed": -6, "time": 2.25},
    ],
    2: [
        {"angle": 25, "speed": -6, "time": 1.3},
        {"angle": -25, "speed": -6, "time": 1.67},
        {"angle": 25, "speed": -6, "time": 0.2},
    ],
}

def overcar_handle(car, overcar):
    global overcar_state, overcar_start_time, is_overcar_handle
    actions = overcar_action.get(overcar, [])
    if overcar_state is None:
        if actions:
            overcar_state = 0
            overcar_start_time = time()  # Initialize overcar_start_time here
            is_overcar_handle = True
            action = actions[overcar_state]
            car.drive(speed=action["speed"], angle=action["angle"])
        return overcar
    
    if overcar_state < len(actions):
        elapsed_time = time() - overcar_start_time
        current_action = actions[overcar_state]
        if elapsed_time >= current_action["time"]:
            overcar_state += 1
            if overcar_state < len(actions):
                next_action = actions[overcar_state]
                overcar_start_time = time()  # Reset the start time for the next action
                car.drive(speed=next_action["speed"], angle=next_action["angle"])
                logging.info(f"Performing action {overcar_state + 1} at intersection {intersection}")
...
if __name__ == '__main__':
    ...
    car = Automobile_Data(trig_control=True, trig_cam=False, trig_gps=False, trig_bno=True, trig_enc=True,
                          trig_sonar=True, trig_sonar1=True)
    maneuver = Maneuvers()
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            elif is_overcar_handle:
                overcar_handle(car, overcar )
            
            elif distance < 0.535 and intersection == 1:
                
              overcar += 1
              if overcar < 3:
                overcar = overcar_handle(car, overcar)
              else:
                car.stop()
            elif distance <0.5:
             car.stop()
